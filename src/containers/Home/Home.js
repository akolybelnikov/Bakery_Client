...
export default class Home extends Component {
  ...
  async componentDidMount() {
    try {
      ...
      this.setState({offerimage: offer.image, offercontent: offer.content})
    } catch (e) {
      console.log(e)
    }
  }
  ...
  async getProducts() {
    const hours = 3600000
    const timecheck = Date.now() - 120 * hours
    const results = []
    const timestamp = {
      createdAt: Date.now()
    }
    try {
      const fingerprint = await localforage.getItem("productstimestamp")
      ...
      if (products && fingerprint && fingerprint.createdAt > timecheck) {
        ...
        if (fingerprint) {
          await localforage.removeItem("productstimestamp")
        }
        ...
        results.push(bread, coffee, cakes, order);

        const allProducts = results.reduce((a, b) => a.concat(b), []);
  
        await localforage.setItem("products", allProducts)
        await localforage.setItem("productstimestamp", timestamp)
      }
    } catch (e) {
...
async getOffer() {
  ...
  try {
    ...
    if (offer && fingerprint && fingerprint.createdAt > timecheck) {
      ...
      if (fingerprint) {
        await localforage.removeItem("offertimestamp")
      }
...
openWarningNotification() {
  notification["warning"]({message: "Произошла ошибка при загрузке! Возможно нет связи с интернетом.", description: "Пожалуйста, попробуйте загрузить приложение позже."});
}
    ...
    <CategoryCard
      ...
      <Card
        ...
        <CategoryImage
          ...
          placeholder={category.categoryName === "bread"
          ? tinyBread
          : category.categoryName === "coffee"
            ...
            : category.categoryName === "cakes"
              ? tinyCakes
              : tinyOrder}
          src={category.categoryName === "bread"
