...
export default class Instafeed extends React.Component {
  ...
  async fetchPosts() {
    ...
    try {
      const fingerprint = await localforage.getItem("instatimestamp");

      const posts = await localforage.getItem("posts");
      if (posts && fingerprint && fingerprint.createdAt > timecheck) {
        return posts;
      } else {
        if (!fingerprint) {
          const timestamp = { createdAt: Date.now() };
          await localforage.setItem("instatimestamp", timestamp);
        }

        if (posts) {
          await localforage.removeItem("posts");
        }
